...
This is a **CAIP-10-compliant cross-blockchain registry** for tracking wallet addresses and contracts, with chain-qualified transaction tracking, to reveal transparency trails that might be used for various information purposes.
...
2. **Cooldown** - The announcement gives a randomized cooldown (a few minutes) followed by a registration window (on the order of ten minutes) — see [Two-Phase Registration](/concepts/two-phase-registration) for the per-chain figures as deployed
