...
This is a **CAIP-10 compliant cross-blockchain registry** for tracking these different addresses and transactions in order to reveal transparency trails that might be used for various information purposes.
...
Storing this type of data and associated metadata along with it will allow things like off-ramps to coordinate around tracking down stolen funds, or wallets to eventually have another reference resource for untrustable smart contracts.
...
Other types of attacks like **address poisoning** occur often where the user does not lose COMPLETE control of their wallet, but suffers from a fraudulent irreversible transaction in itself.
