...
sequenceDiagram
    ...
    W->>C: register(registeree, trustedForwarder, ...)
...
Anyone can submit the transaction (the trusted forwarder is named in the signature). This is what enables relay registration methods.
...
Called by `FraudRegistryHub` when a cross-chain message arrives from a `SpokeRegistry`. Silently succeeds if the wallet is already registered (no revert). The entry struct stores only `registeredAt`, `incidentTimestamp`, `bridgeId`, and `isSponsored`. Cross-chain provenance fields (`reportedChainId`, `sourceChainId`, `messageId`) are emitted in events only -- they are not written to storage.
...
| `getSignatureDeadline(...)`          | Signing deadline for frontend signature generation                   |
