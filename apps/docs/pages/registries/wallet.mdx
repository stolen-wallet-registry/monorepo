...
struct WalletEntry {
    ...
    uint32  batchId;            // Operator batch ID (0 for individual/cross-chain)
    uint8   bridgeId;           // 0 = local, 1 = Hyperlane
    bool    isSponsored;        // True if someone else paid gas
}
```

The struct is 22 bytes, fitting in a single EVM storage slot. This is a deliberate design choice: storage carries minimal data per entry, while events carry rich provenance data (chain IDs, message IDs, reporters, etc.).
