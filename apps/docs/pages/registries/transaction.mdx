...
Transaction registration uses a commit-reveal scheme to prevent front-running. During the acknowledge phase, the reporter commits to a hash of the batch without revealing which transactions are being reported.
...
**Why this matters:** Without the commit-reveal, a front-runner could see the acknowledge transaction in the mempool, extract the transaction hashes, and register them first. The dataHash ensures the contents stay hidden until the registration phase.
