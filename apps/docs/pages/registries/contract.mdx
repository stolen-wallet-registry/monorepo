...
struct ContractEntry {
    ...
    bytes32 reportedChainId;   // CAIP-2 hash of chain where contract is deployed
    bytes32 operatorId;        // Operator that registered this entry
    uint256 batchId;           // Batch this entry belongs to
}
```

Every entry tracks its `batchId` directly in the struct. This is different from the other registries where batch linking is done at the indexer level via transaction hash matching.
