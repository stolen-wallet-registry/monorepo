...
- Willingness to pay for someone's registration
...
3. Wait ◄────────── connect ──────────► Dial peer ID

4. Sign ACK ────── /swr/ack/sig ──────► Store signature
          ◄─── /swr/ack/rec ────────── Confirm received
          ◄─── /swr/ack/pay ────────── Submit tx, send hash

5. ─────────────── grace period ───────────────────────

6. Sign REG ────── /swr/reg/sig ──────► Store signature
          ◄─── /swr/reg/rec ────────── Confirm received
          ◄─── /swr/reg/pay ────────── Submit tx, send hash
...
- libp2p uses keep-alive pings every 45 seconds
...
- If lost, you may need to restart from acknowledgement

## Security Considerations

- **Verify the helper** - Only use someone you trust
- **Check addresses** - Verify the registeree address before signing
- **Don't share signatures** - They can only be used for registration
- **Public networks** - P2P traffic is encrypted but use secure channels for peer IDs
