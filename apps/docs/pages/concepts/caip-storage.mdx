...
### [CAIP-363](https://standards.chainagnostic.org/CAIPs/caip-363): Wildcard Chain References
...
```
eip155:_:0x742D35CC6634c0532925A3b844BC9E7595F0BEb0
```
...
**Generic wallets** - the namespace-dispatching entry point. EVM takes the wildcard branch; everything else is chain-specific:
    ...
    internal pure returns (bytes32)
{
    if (namespaceHash == NAMESPACE_EIP155) {
        // EVM: wildcard key (same wallet on all EVM chains) - chainRef ignored
...
:::warning
`walletKey`'s EVM branch hashes the **pre-hashed constants** `NAMESPACE_EIP155` and `WILDCARD`, while `evmWalletKey` hashes the **literal string** `"eip155:_:"`. These produce **different keys for the same wallet**. Every live EVM path uses `evmWalletKey` — `acknowledge`, `register`, the operator batch path, and the EVM branch of `registerFromHub` all call it directly. `walletKey` is reached only for non-EVM namespaces arriving via cross-chain messages, where `chainRefHash` is pre-computed by the spoke. Do not mix the two when computing a key off-chain.
:::
