...
### CAIP-363: Wildcard Chain References
...
Every identifier maps to a `bytes32` storage key via `keccak256`. This gives O(1) lookup with no merkle trees, linked lists, or complex indexing.
...
**Single-read verification.** To check if a wallet is registered, compute the key and read one storage slot. No iteration, no merkle proofs, no indexer dependency for on-chain checks.
