...
3. The registration window expires (~10 minutes after acknowledgement) — the attacker must trick the victim into signing a second transaction within this narrow window or the acknowledgement becomes invalid
...
The registration window is the time between the end of the grace period and the acknowledgement deadline. This is typically ~8-10 minutes. If the user does not complete the second signature within this window, the acknowledgement becomes invalid and the entire process must restart from Phase 1. This expiry is central to the social engineering protection — an attacker has a narrow, non-renewable window to coerce a second signature.
