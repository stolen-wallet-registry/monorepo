...
:::warning
Two-phase registration is an added layer of social engineering protection, not a guarantee. A sufficiently motivated attacker with sustained access to a victim could still complete both phases. The design raises the bar significantly - it does not eliminate all risk.
:::
...
:::info
The `trustedForwarder` address is never emitted in on-chain events. This is intentional - it protects the privacy of relay helpers. The only public signal is the `isSponsored` boolean, which indicates that someone other than the wallet owner paid gas without revealing who.
:::
