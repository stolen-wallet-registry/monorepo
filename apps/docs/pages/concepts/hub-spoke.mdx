...
| FeeManager              | Yes        | Yes          |
| WalletSoulbound         | Yes        | -            |
| SupportSoulbound        | Yes        | -            |
| SoulboundReceiver       | Yes        | -            |
| TranslationRegistry     | Yes        | -            |
| SpokeRegistry           | -          | Yes          |
| SpokeSoulboundForwarder | -          | Yes          |
| Bridge Adapter          | -          | Yes          |
...
sequenceDiagram
    ...
    F->>R: registerFromHub()
    R->>R: Store entry (bridgeId=1)<br/>Emit events with messageId, sourceChainId
...
9. **Registry stores** a minimal entry (`bridgeId=1`, `registeredAt`, etc.) and emits events with the full cross-chain provenance (`sourceChainId`, `messageId`, `reportedChainId`)

Cross-chain provenance fields (`sourceChainId`, `messageId`, `reportedChainId`) are not stored in the entry struct -- they are emitted in `CrossChainWalletRegistered` and `CrossChainTransactionRegistered` events. The indexer reads these events to reconstruct the full cross-chain message trail. On-chain lookups (`isWalletRegistered`, `isTransactionRegistered`) only need the storage key and minimal entry data.
...
function register(
    ...
    uint256 deadline,
    uint256 nonce,
    uint256 windowBlock,   // NOT signed - binds via the signed windowBlockHash
    uint8 v, bytes32 r, bytes32 s
) external payable;
```

`register()` takes ten parameters. The `windowBlock` is the block whose hash the registration signature committed to — the anti-phishing control described in [Two-Phase Registration](/concepts/two-phase-registration#signature-freshness-the-primary-control). It travels as unsigned calldata because the signed `windowBlockHash` already binds it.
...
:::info
The hub names its first parameter `registeree` and the spoke names it `wallet`. Parameter names are not part of the ABI, so the function selectors are identical and the unification holds. Internally `SpokeRegistry.register` immediately collapses its arguments into a memory struct to stay under the EVM's 16-slot stack limit (this project builds without via-ir) — the external signature stays flat and byte-identical to the hub's.
:::
