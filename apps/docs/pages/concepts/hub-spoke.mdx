...
Users interact with a SpokeRegistry on their chain. The spoke sends a cross-chain message via Hyperlane to the hub, where the entry is permanently stored.
...
sequenceDiagram
    ...
    S->>H: sendMessage via Adapter
    H-->>I: Deliver to hub chain
...
4. **HyperlaneAdapter sends** the message via the Hyperlane mailbox
5. **Hyperlane relayer delivers** the message to the hub chain's `CrossChainInbox`
...
Only messages from trusted spoke registries on trusted chains are processed. The inbox also performs defense-in-depth validation: the `sourceChainId` inside the payload must match the Hyperlane origin domain.
