...
| **Wormhole**       | 30+ chains, non-EVM support  | Past security incident |
...
struct CrossChainRegistration {
    bytes32 caip10Address;    // CAIP-10 formatted
    bytes32 signatureHash;    // For verification
    uint256 timestamp;
    address submitter;
}
```
...
- CAIP-10 includes chainId, preventing cross-chain replay
- Nonces are per-chain, per-wallet
- Deadlines are chain-specific (block numbers differ)
