...
The fundamental design decision: store every entry on-chain as a full struct rather than relying on off-chain aggregation. L2 gas economics make per-entry storage practical.
...
At typical Base L2 execution fees (varies with network conditions):
...
A 1,000-entry batch uses 21.5M gas - about 5.7% of a single Base block. Gas cost: $0.065. The protocol fee ($25) is 380x the gas cost in this example.

L2 storage costs are low enough that storing full structs per entry is practical. Direct on-chain queryability and readability are worth the extra storage.
