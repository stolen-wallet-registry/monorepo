...
graph TB
    ...
    subgraph Hub Chain
        Sub[OperatorSubmitter]
        WR[WalletRegistry]
        TR[TransactionRegistry]
        CR[ContractRegistry]
        Hub[FraudRegistryHub<br/>query router]
    end
    ...
    WebApp -->|register| WR
    WebApp -->|register| TR
    WebApp -->|register| Spoke
    CLI -->|batch submit| Sub
    Sub --> WR
    Sub --> TR
    Sub --> CR
    Spoke --> Adapter
    ...
    Inbox --> Hub
    Hub -->|registerFromHub| WR
    Hub -->|registerFromHub| TR
    WR --> Indexer
    TR --> Indexer
    CR --> Indexer
    Indexer --> Dashboard
    Dashboard -.->|isRegistered| Hub

`} />

Individual users write **directly to the registry contracts** — on the hub chain to `WalletRegistry` or `TransactionRegistry`, on a spoke chain to `SpokeRegistry`. Operators write through `OperatorSubmitter`, which checks their capability bits before forwarding the batch. `FraudRegistryHub` is not on the individual write path at all: it is the cross-chain receiver (routing inbound spoke messages to the right registry) and the unified read interface (`isRegistered`).

## Security Model

Every individual wallet and transaction registration requires **two separate signatures** with a randomized delay between them. This two-phase EIP-712 flow prevents single-transaction phishing attacks - an attacker cannot trick a victim into registering a legitimate wallet in one step.
