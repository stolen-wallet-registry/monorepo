...
Get the Stolen Wallet Registry running locally. Two modes available: **single-chain** (simpler, one Anvil instance) and **cross-chain** (two Anvil chains + Hyperlane relayer).
...
Starts a local Ethereum node on `http://localhost:8545` with chain ID `31337` and 13-second block times.
...
Deploys all hub contracts (FraudRegistryHub, WalletRegistry, TransactionRegistry, ContractRegistry, OperatorSubmitter) and wires them together. Contract addresses are logged to the console.
...
For testing hub-spoke architecture with Hyperlane message passing between two local chains.

### Terminal 1 -- Anvil + Hyperlane Relayer
...
- **Hub chain** on port 8545 (chain ID 31337, 12s blocks)
- **Spoke chain** on port 8546 (chain ID 31338, 12s blocks)
- **Hyperlane relayer** that auto-relays cross-chain messages

:::info
The Anvil instances load pre-deployed Hyperlane state from `.anvil-state/`. The relayer starts after a 3-second delay to let the chains initialize.
:::
...
Deploys hub contracts to chain 31337, spoke contracts to chain 31338, configures trust relationships, and seeds test operator data.
...
Starts the frontend with cross-chain features enabled (`VITE_CROSSCHAIN=true`).
