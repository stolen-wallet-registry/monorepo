...
| ------- | -------------- | ----------------------------------------------------------- |
| Node.js | 22.x (exactly) | [nodejs.org](https://nodejs.org)                            |
| pnpm    | 9+             | `npm install -g pnpm`                                       |
| Foundry | Latest         | `curl -L https://foundry.paradigm.xyz \| bash && foundryup` |

:::warning
Node must be **major version 22** — `engines` pins `>=22.12.0 <23` and a `preinstall` gate fails the install on anything else, including Node 23 and 24. Node 24 ships experimental Web Storage that shadows jsdom, so the web test suite would run against the wrong globals. Run `nvm use` to pick up the repo's `.nvmrc`.
:::
...
Build contracts and generate the TypeScript ABI exports:

```bash
pnpm export-abi
```

Run this **from the repo root**. It builds the contracts through turbo and then formats the generated files with Prettier. `pnpm forge:build` only runs `forge build` — it compiles the contracts but does not regenerate `packages/abis`, and the frontend consumes those exports, so stale ABIs cause silent runtime failures.

:::info
Re-run `pnpm export-abi` after **any** Solidity change. Two CI jobs guard this: `abi-drift` regenerates and fails on a diff, `abi-freshness` fails if `packages/contracts/src/**.sol` changed without `packages/abis/src` changing alongside it.
:::
...
Both chains start with **12s** interval mining. `deploy:crosschain` (next step) switches them to automine so the deployment lands quickly, then restores interval mining at **13s** — so the steady-state block time you develop against is 13s, matching the `(2, 50)` grace/deadline block counts the local deployment uses.
